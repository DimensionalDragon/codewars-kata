function dblLinear(n) {
    const LIMIT = 1000000;
    const sequence = new Array(LIMIT).fill(0); // Create container for the sequence

    let doublePlusOneIndex = 0, triplePlusOneIndex = 0; // Index that points to the last numbers generated, both start at 0
    let doublePlusOne = 1, triplePlusOne = 1; // Latest numbers generated

    for (let i = 0; i < LIMIT; i++ ) // Generate the sequence
    {
        ...
        const minimum = (doublePlusOne <= triplePlusOne) ? doublePlusOne : triplePlusOne;
